...
<div class="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
    <div class="bg-white rounded-lg shadow-md overflow-hidden">
        <div class="px-6 py-4 bg-gray-50 border-b border-gray-200 flex justify-between items-center">
            <h2 class="text-xl font-semibold text-gray-800">Tambah Perangkat Desa</h2>
            <a href="{{ route('perangkat_desa.index') }}"
               class="text-gray-500 hover:text-gray-700 font-medium text-sm">Kembali</a>
        </div>

        <form action="{{ route('perangkat_desa.store') }}" method="POST" class="p-6">
            @csrf

            @if(session('success'))
            <div class="mb-4 bg-green-50 text-green-700 p-3 rounded">{{ session('success') }}</div>
            @endif
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="kode_jabatan"
                        ...
                        'kode_jabatan') == 'PD06' ? 'selected' : '' }}>Kasi
                        Pemerintahan</option>
                        <option value="PD78" {{ old(
                        'kode_jabatan') == 'PD78' ? 'selected' : '' }}>Kasi Kesejahteraan dan
                        Pelayanan</option>
